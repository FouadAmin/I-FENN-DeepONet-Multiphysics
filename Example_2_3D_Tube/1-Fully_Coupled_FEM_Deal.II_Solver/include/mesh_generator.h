...
namespace mesh_generator_space
{
    ...
    public:
        ...
        static void generate_cylinder_2(Triangulation<dim> &triangulation, double inner_radius, double outer_radius, double cylinder_length, unsigned int n_radial_cells, unsigned int n_axial_cells, unsigned int refinement_level)
        {
            GridGenerator::cylinder_shell(triangulation,
                                          cylinder_length,
                                          inner_radius,
                                          outer_radius, n_radial_cells, n_axial_cells);
            for (const auto &cell : triangulation.active_cell_iterators())
                for (const auto &face : cell->face_iterators())
                    if (face->at_boundary())
                    {
                        ...
                        if (std::fabs(face_center[2] - 0) < 1e-9)
                            face->set_boundary_id(1); // -ve Z face
                        else if (std::fabs(face_center[2] - cylinder_length) < 1e-9)
                            face->set_boundary_id(2); // +ve Z face
                        else if (std::sqrt(face_center[0] * face_center[0] +
                                           face_center[1] * face_center[1]) <
                                 (inner_radius + outer_radius) / 2)
                            face->set_boundary_id(3); // Inner face
                        else
                            face->set_boundary_id(4); // Outer face
                    }
