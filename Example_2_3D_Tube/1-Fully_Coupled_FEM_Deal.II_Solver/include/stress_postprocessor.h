#ifndef STRESS_POSTPROCESSOR_H // Include guard to prevent multiple inclusions
#define STRESS_POSTPROCESSOR_H
...
namespace stress_postprocessor_space
{
  ...
  public:
    ...
    StressPostProcessor(const std::string vector_name, StressCalculator<dim> stress_calculator) : DataPostprocessorTensor<dim>(vector_name, update_gradients)
    {
                          ...
                          std::vector<Vector<double>> &computed_quantities) const override
    {
      ...
      for (unsigned int p = 0; p < input_data.solution_gradients.size(); ++p)
      {
        ...
        for (unsigned int d = 0; d < dim; ++d)
          for (unsigned int e = 0; e < dim; ++e)
            computed_quantities[p][Tensor<2, dim>::component_to_unrolled_index(TableIndices<2>(d, e))] = stress_tensor[d][e];
      }
    }
