...
import shutil
import Module_DL_Helper_Tube_008 as MDLH
import Module_Solution_Loader_Helper_Tube_002 as MSLH
import mmap
import posix_ipc
import struct
import time
...
def load_shared_memories():
    ...
    shared_output_format = "5184f"  # n floats
...
def start_input_out_process():
    ...
    while True:
        ...
        check_vector = torch.tensor(check_vector)
        max_check = torch.max(check_vector)
        min_check = torch.min(check_vector)
        if not are_equal(max_check, 1.0) or not are_equal(min_check, 1.0):
            print(f"max_check: {max_check}, min_check: {min_check}")
            # loop over the check_vector and print the non 1.0 values
            # open special log file to write the check_vector values
            check_vector_log_file = f'check_vector_log.log'
            with open(check_vector_log_file, 'w') as f:
                for i, val in enumerate(check_vector):
                    cells_per_a = 32
                    cells_per_r = 4
                    cells_per_z = 4
                    a_index = i // (cells_per_r * cells_per_z)
                    r_index = (i % (cells_per_r * cells_per_z)) // cells_per_z
                    z_index = (i % (cells_per_r * cells_per_z)) % cells_per_z
                    # print(f"check_vector[{i}]=>[{a_index},{r_index},{z_index}]: {val}")
                    f.write(f"check_vector[{i}]=>[{a_index},{r_index},{z_index}]: {val}\n")
        ...
        #######################################################################
        my_logger.add_to_wait_list(f"Received check_vector => size: {check_vector.shape} max: {max(check_vector)} min: {min(check_vector)}")
        ...
        if t_step_no == 120:
            ...
            time_step_list = np.arange(1, 121, 1)
            n_time_steps = len(time_step_list)
            dealii_disp_array_grid = np.zeros((n_time_steps, 64, 9, 9, 3))
            coords_grid_i = None
            for ii, time_index in enumerate(time_step_list):
                ...
                data_array_i, coords_grid_i = MSLH.custom_get_data(args_load_case_file_name_lc, time_index, False, fem_readpath)
            ...
            deepOnet_theta_array_grid = predicted_output_data_up_to_current_denormalized.cpu().numpy().reshape(120,64,9,9,1)
            ...
            combined_true_array = output_data_i_full_denormalized.reshape(120, 64, 9, 9, 4).cpu().numpy()
            ...
            MDLH.plot_true_vs_prediction_3D(combined_true_array, combined_predicted_array,coords_grid_i, p_dict["target_load_case_index"], init_save_name, 'I-FENN', to_use_plots_save_folder)
...
def load_data_for_a_specific_load_case(load_case_index):
    ...
    load_file_name = f'Model010_Tube_flux_gen_matrix_v001'
    output_file_name = f'all_data_array_grid_LC-'
    strain_file_name = f'all_strain_array_grid_LC-'

    coordinates_file_name = f'coordinates_array_grid_LC-0-50'
    ...
    class DeepONet(nn.Module):
        ...
        def forward(self, load_input, strain_input ,trunk_input):
            ...
            if enforce_BC:
                ...
                z_values = trunk_input[:,2]
                z_function_NN = torch.tanh((z_values+0)*10)
                x_function_NN_reshaped = z_function_NN.view(1,1,-1,1)
...
load_case_file_name = 'Model011_V001-tests_lcxxx_mesh3_t60000_inc120_dt500_stxxx.h5'
