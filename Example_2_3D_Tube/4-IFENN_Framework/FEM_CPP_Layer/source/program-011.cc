...
namespace Program011
{
  ...
  public:
    ...
    std::string solution_file_name = "Model011_V001-tests";
    ...
    static const int n_temperature_vector = 5184; // 64 x 9 x 9
    static const int n_strain_cells_skip_a = 2;
    static const int n_strain_cells_skip_r = 2;
    static const int n_strain_cells_skip_z = 2;
    static const int n_strain_cells_per_a = 64;
    static const int n_strain_cells_per_r = 8;
    static const int n_strain_cells_per_z = 8;
    static const int n_strain_vector = 512; // 32 x 4 x 4

    static const int selected_cells_per_a = 32;
    static const int selected_cells_per_r = 4;
    static const int selected_cells_per_z = 4;
    static const int selected_cells_per_rz = 4 * 4;
  ...
  void TopLevel<dim>::assemble_system()
  {
    ...
    FESystem<dim> fe_temperature (FE_Q<dim>(1)^1); // used for mapping temperature values from nodes to quads

    FEValues<dim> fe_values_temperature(fe, quad_formula_cell,
                                        update_values | update_gradients |
                                        update_quadrature_points | update_JxW_values);
    ...
    QuadTemperatureProvider<dim> quad_temperature_provider(t_current, t_dt, t_step_no, temperature_input_array);
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      if (cell->is_locally_owned())
      {
        ...
        quad_temperature_provider.value_list_custom(fe, fe_values,cell,fe_values.get_quadrature_points(), quad_temperature_values);
  ...
  void TopLevel<dim>::update_temperature_input_array()
  {
    ...
    double cell_size_per_a = (2 * M_PI) / (double)n_strain_cells_per_a;
    double cell_size_per_r = 1.0 / (double)n_strain_cells_per_r;
    double cell_size_per_z = 1.0 / (double)n_strain_cells_per_z;

    double skip_cell_size_a = cell_size_per_a * n_strain_cells_skip_a;
    double skip_cell_size_r = cell_size_per_r * n_strain_cells_skip_r;
    double skip_cell_size_z = cell_size_per_z * n_strain_cells_skip_z;
    double inside_cell_tolerance = 0.1;
    double skip_tolerance_a = cell_size_per_a * inside_cell_tolerance;
    double skip_tolerance_r = cell_size_per_r * inside_cell_tolerance;
    double skip_tolerance_z = cell_size_per_z * inside_cell_tolerance;
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      if (cell->is_locally_owned())
      {
        ...
        double midpoint_x = cell_midpoint[0];
        double midpoint_y = cell_midpoint[1];
        double midpoint_z = cell_midpoint[2];
        double midpoint_r = std::sqrt(midpoint_x * midpoint_x + midpoint_y * midpoint_y);
        double midpoint_a = std::atan2(midpoint_y, midpoint_x);

        double edge_a = midpoint_a - cell_size_per_a / 2;
        double edge_r = midpoint_r - cell_size_per_r / 2;
        double edge_z = midpoint_z - cell_size_per_z / 2;
        
        // if abs angle is close to 0, convert to 0
        if (std::abs(edge_a) < (2 * M_PI * 1e-3))
          edge_a = 0;
        // if angle is negative, convert to positive
        if (edge_a < 0)
          edge_a += 2 * M_PI;

        double remainder_a = std::fmod(edge_a, skip_cell_size_a);
        double remainder_r = std::fmod(edge_r-1, skip_cell_size_r);
        double remainder_z = std::fmod(edge_z, skip_cell_size_z);

        bool remainder_a_close = helper_functions_space::is_remainder_close<dim>(remainder_a, skip_cell_size_a, skip_tolerance_a);
        bool remainder_r_close = helper_functions_space::is_remainder_close<dim>(remainder_r, skip_cell_size_r, skip_tolerance_r);
        bool remainder_z_close = helper_functions_space::is_remainder_close<dim>(remainder_z, skip_cell_size_z, skip_tolerance_z);

        bool is_vertex_on_skip = remainder_a_close && remainder_r_close && remainder_z_close;
        std::ofstream skip_data_log("logs/skip_log.log", std::ios::app);
        skip_data_log
            << "t_step_no: " << t_step_no
            << " t_current: " << t_current
            << " t_dt: " << t_dt
            ...
            << " midpoint_a: " << midpoint_a * 180 / M_PI
            << " midpoint_r: " << midpoint_r
            << " midpoint_z: " << midpoint_z
            << " edge_a: " << edge_a * 180 / M_PI
            << " edge_r: " << edge_r
            << " edge_z: " << edge_z
            << " remainder_a: " << remainder_a * 180 / M_PI
            << " remainder_r: " << remainder_r
            << " remainder_z: " << remainder_z
            << " remainder_a_close: " << remainder_a_close
            << " remainder_r_close: " << remainder_r_close
            << " remainder_z_close: " << remainder_z_close
            << " is_vertex_on_skip: " << is_vertex_on_skip
            << std::endl;
        if (is_vertex_on_skip)
        {
          ...
          int rounded_a_index = (int)std::round(edge_a / skip_cell_size_a);
          int rounded_r_index = (int)std::round((edge_r-1) / skip_cell_size_r);
          int rounded_z_index = (int)std::round(edge_z / skip_cell_size_z);
          int shared_index = rounded_a_index * selected_cells_per_rz + rounded_r_index * selected_cells_per_z + rounded_z_index;

          shared_input->strain_vector[shared_index] = strain_trace_midpoint;
          shared_check->repetition_vector[shared_index] += 1;

          // Temp Check on calculated strain
          if ((rounded_a_index == 0 || rounded_a_index == 1) && rounded_r_index == 0 && rounded_z_index == 0)
          {
    ...
    int dim_r = temperature_input_array[0][0].size();
    int dim_z = temperature_input_array[0][0][0].size();
    for (unsigned int shared_output_index = 0; shared_output_index < n_temperature_vector; shared_output_index++)
    {
      int a_index = shared_output_index / (dim_r * dim_z);
      int r_index = (shared_output_index % (dim_r * dim_z)) / dim_z;
      int z_index = (shared_output_index % (dim_r * dim_z)) % dim_z;
      temperature_input_array[t_step_no][a_index][r_index][z_index] = shared_output->temperature_vector[shared_output_index];
    }
  }
...
void run_one_elastic_problem(int load_case_index, DoubleVector4D temperature_input_array, int mesh_refinement, double total_load_time, int load_increments, std::string arg_solution_folder)
{
  using namespace dealii;
  using namespace Program011;
...
int main(int argc, char **argv)
{
  try
  {
    ...
    int new_vector_angle_size = 8 * std::pow(2, mesh_refinement);
    int new_vector_r_size = std::pow(2, mesh_refinement) + 1;
    int new_vector_z_size = std::pow(2, mesh_refinement) + 1;

    DoubleVector4D interpolated_temperature_array(new_vector_time_size, DoubleVector3D(new_vector_angle_size, DoubleVector2D(new_vector_r_size, DoubleVector1D(new_vector_z_size, 0.0))));
    ...
    run_one_elastic_problem(load_case_index, interpolated_temperature_array, mesh_refinement, t_total_load, t_load_increments_no, arg_solution_folder);
  }
