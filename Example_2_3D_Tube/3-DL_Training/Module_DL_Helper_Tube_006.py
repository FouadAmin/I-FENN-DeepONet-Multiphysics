...
def get_generated_flux_for_load_case(load_gen_data, load_case_index, n_angle):
    load_case_data = load_gen_data[load_case_index]
    omega_t_i = load_case_data[0]
    q_inner_0_i = load_case_data[1]
    q_inner_1_i = load_case_data[2]
    omega_inner_r_i = load_case_data[3]
    q_outer_0_i = load_case_data[4]
    q_outer_1_i = load_case_data[5]
    omega_outer_r_i = load_case_data[6]

    time_steps = 121
    t_end = 60000
    t = np.linspace(0, t_end, time_steps, endpoint=True)
    
    # generate flux
    angle = np.linspace(0, 2 * np.pi, n_angle, endpoint=False)
    
    temporal_frequency = omega_t_i * ((2 * np.pi) / 60000)
    temporal_cos = np.cos(temporal_frequency * t)
    inner_radial_cos = np.cos(omega_inner_r_i * angle)
    outer_radial_cos = np.cos(omega_outer_r_i * angle)
    
    flux_inner = np.zeros((time_steps, n_angle))
    flux_outer = np.zeros((time_steps, n_angle))
    
    for i_t in range(time_steps):
        flux_inner[i_t] = q_inner_0_i + q_inner_1_i * temporal_cos[i_t] * inner_radial_cos
        flux_outer[i_t] = q_outer_0_i + q_outer_1_i * temporal_cos[i_t] * outer_radial_cos

    # concatenate inner and outer flux
    all_flux = np.concatenate((flux_inner, flux_outer), axis=1)
    
    return all_flux, flux_inner, flux_outer, t, angle
...
class MyDataset(TorchDataset):
    def __init__(self, dl_settings, mode):
        ...
        self.load_stats = read_pickle_data(f'{self.load_folder_path}/Model010_all_flux_stats.pkl')
        ...
        for dataset_index, global_index in enumerate(range(self.LC_start, self.LC_end)):
            group_index, group_start, group_end, local_index = get_group_start_end_indices(global_index, 50)
    ...
    def pre_load_all_data(self):
        ...
        load_n_per_side = self.slicing_settings['point_load_n_per_side']
            ...
            with h5py.File(output_file_path, 'r') as output_file, h5py.File(strain_file_path, 'r') as strain_file:
                if self.read_all_the_file_once:
                    ...
                    for _,_,_, local_index, dataset_index, global_index in required_data_in_group:
                        load_data = get_generated_flux_for_load_case(self.load_gen_data, global_index, load_n_per_side)[0][load_time_start:load_time_end:load_time_skip,:]
    ...
    def get_sliced_coordinates_array_grid(self):
        output_start = self.slicing_settings['point_output_start']
        output_skip = self.slicing_settings['point_output_skip']
        output_end = self.slicing_settings['point_output_end']
    ...
    def get_coordinates_array(self, return_as_tensor):
        sliced_coordinates_array = self.get_sliced_coordinates_array_grid()
        if self.slicing_settings['point_output_flatten']:
...
def get_pyvista_grid_from_data(values_grid, coordinates_grid):
    
    n_a = values_grid.shape[0]
    n_r = values_grid.shape[1]
    n_z = values_grid.shape[2]
    
    data_array = values_grid.reshape(-1)
    points_coords = coordinates_grid.reshape(-1, 3)
    ...
    if n_points_coords != n_points_data:
        raise ValueError(f'Error: at 3d plot n_points_coords={n_points_coords} != n_points_data={n_points_data}')
    
    cells_list = []
    cell_types_list = []

    def get_node_index(i_a, i_r, i_z):
        return i_a * n_r * n_z + i_r * n_z + i_z
        ...
        for i_r in range(n_r-1):
            for i_z in range(n_z-1):
                ...
                indices_array = np.array([
                    get_node_index(i_a_0, i_r_0, i_z_0),
                    get_node_index(i_a_0, i_r_1, i_z_0),
                    get_node_index(i_a_1, i_r_1, i_z_0),
                    get_node_index(i_a_1, i_r_0, i_z_0),
                    get_node_index(i_a_0, i_r_0, i_z_1),
                    get_node_index(i_a_0, i_r_1, i_z_1),
                    get_node_index(i_a_1, i_r_1, i_z_1),
                    get_node_index(i_a_1, i_r_0, i_z_1),
                    ])
                cell = np.array([8, *indices_array])
                cells_list.append(cell)
                cell_types_list.append(12)
...
def plot_true_vs_prediction_3D(true_output, predictions, coordinates_grid, history_index, description, trial_save_number, plots_save_folder):
    # know the dimensions
    n_time_steps = true_output.shape[0]
    angle_dim = true_output.shape[1]
    r_dim = true_output.shape[2]
    ...
    component_labels = [' Temperature  ', 'Displacement x', 'Displacement y', 'Displacement z']
    sub_plot_no_list = ['(a)', '(b)', '(c)', '(d)', '(e)', '(f)', '(g)', '(h)', '(i)', '(j)', '(k)', '(l)', '(m)', '(n)', '(o)', '(p)', '(q)', '(r)', '(s)', '(t)', '(u)', '(v)', '(w)', '(x)', '(y)', '(z)']

    time_indices_to_plot = [59,119]
    ...
    for selected_use_slice_option in [False]:
        for i_time in time_indices_to_plot:
            ...
            for n_rows in [ 3 , 4 ]:
                ...
                # Create a new figure
                if n_components == 1:
                    fig_width = int(16/4 * 150)
                else:
                    fig_width = int(16 * 150)
                ...
                for i_component in range(n_components):
                    for i_row in range(n_rows):
                        ...
                        grid = get_pyvista_grid_from_data(solid_to_plot, coordinates_grid)
                        ...
                        p.add_text(sub_plot_no, font_size=30, position=(fig_width_per_component * 0.44,fig_height_per_row * 0.72))
                        if i_component == 0:
                            p.add_text(data_labels[i_row], font_size=30, position=(fig_width_per_component * 0.11,fig_height_per_row * 0.35), orientation=90)
                        if i_row == 0:
                            p.add_text(component_labels[i_component], font_size=30, position=(fig_width_per_component * 0.17,fig_height_per_row * 0.84))
                        ...
                        if i_component == 0 and i_row == 0:
                            p.add_axes(viewport=(-0.10, 0.54, 0.30, 0.94),xlabel='X', ylabel='Y', zlabel='Z', color='black')
                ...
                save_name = f'b-3d_{description}_hist_{history_index}_{save_index_append_zeros}_time_{relative_t_index}{add_name}.png'             
...
def inspect_results_for_model_and_dataloader(training_data_dict, model_path, dataloader, description , mode):
    ...
    coordinates_grid = current_dataset.get_sliced_coordinates_array_grid()
    my_logger.add_to_wait_list(f'coordinates_grid.shape: {coordinates_grid.shape}')
    ...
    for history_index in history_indices:
        ...
        plot_true_vs_prediction_3D(unflattened_true_output[history_index], unflattened_predictions[history_index], coordinates_grid, history_index, description, trial_save_number, plots_save_folder)
