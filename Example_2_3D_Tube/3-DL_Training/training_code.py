...
import shutil
import Module_DL_Helper_Tube_006 as MDLH
...
epochs_sessions_list = [100,4000-100,4000,4000,4000,4000,4000]
...
last_model_path=f'./{models_save_folder}/model_010_{trial_save_text}_last_train.pth'
best_model_path_train=f'./{models_save_folder}/model_010_{trial_save_text}_best_train.pth'
best_model_path_val = f'./{models_save_folder}/model_010_{trial_save_text}_best_val.pth'

previous_model_path = f'./{models_save_folder}/model_010_{trial_save_text}_last_train.pth' # if extend_on_previous_model is True, it will load the model from this path
...
if is_read_only:
    ...
    plots_save_folder = f'{parent_plots_save_folder}/model_010_{read_save_text}'
    sco_file_path = f'./{sco_save_folder}/model_010_{read_save_text}_sco.py'
    log_file_name = f'model_010_{read_save_text}_log.log'
else:    
    # new model
    plots_save_folder = f'{parent_plots_save_folder}/model_010_{trial_save_text}'
    sco_file_path = f'./{sco_save_folder}/model_010_{trial_save_text}_sco.py'
    log_file_name = f'model_010_{trial_save_text}_log.log'
...
target_coord_list_arz = []
target_coord_list_arz.append(np.array([dim_a*0.125, 1+dim_r*0.0, dim_z*1.0]))
target_coord_list_arz.append(np.array([dim_a*0.125, 1+dim_r*1.0, dim_z*1.0]))
target_coord_list_arz.append(np.array([dim_a*0.125, 1+dim_r*1.0, dim_z*0.0]))

target_coord_list = []
for target_coord_arz in target_coord_list_arz:
    coord_a = target_coord_arz[0]
    coord_r = target_coord_arz[1]
    coord_z = target_coord_arz[2]
    target_coord = np.array([coord_r*np.cos(coord_a), coord_r*np.sin(coord_a), coord_z])
...
scheduler_config_list = [
    ('linear',100  , 2000,1.0*starting_lr, 0.3*starting_lr),
    ('linear',2000 , 4000,0.3*starting_lr, 0.2*starting_lr),
    ('linear',4000 ,24000,0.2*starting_lr, 0.08*starting_lr),
]
