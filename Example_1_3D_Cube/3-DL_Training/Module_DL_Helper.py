...
def plot_true_vs_prediction_slice(true_output, predictions, history_index, description, trial_save_number, plots_save_folder):
    ...
    # use symbols (theta, ux, uy, uz) for the components
    ...
    for i_slice in range(n_slices):
        ...
        for i_component in range(n_components):
            for i_row in range(n_rows):
                ...
                # set title font size
                # axs[i_row,i_component].title.set_fontsize(20)
                # flip the x,y axis
                # axs[i_row,i_component].invert_xaxis()
...
def get_pyvista_grid_from_data(data_array, refinement_level):
    ...
    if refinement_level > 0:
        ...
        # print( f"no zoom applied to data_array.shape = {data_array.shape} -> fine_data_array.shape = {fine_data_array.shape}")
...
def plot_true_vs_prediction_3D(true_output, predictions, history_index, description, trial_save_number, plots_save_folder):
    ...
    time_indices_to_plot = [9,49,74,99]
            ...
            for n_rows in [ 3 , 4 ]:
                ...
                for i_component in range(n_components):
                    for i_row in range(n_rows):
                        ...
                        # # temp normalization
                        # solid_to_plot = solid_to_plot / np.max(np.abs(solid_to_plot))

                        # print(f"solid_to_plot.shape = {solid_to_plot.shape}")
                        # determine max and min values for colorbar
                        ...
                        # text_to_add = component_labels[i_component]
                        # p.add_text(text_to_add, font_size=20, position=(fig_width_per_component * 0.3,fig_height_per_row * 0.85))
                        p.add_text(sub_plot_no, font_size=30, position=(fig_width_per_component * 0.44,fig_height_per_row * (0.77 if n_rows == 3 else 0.79)))
                        if i_component == 0:
                            p.add_text(data_labels[i_row], font_size=30, position=(fig_width_per_component * 0.11,fig_height_per_row * 0.35), orientation=90)
                        if i_row == 0:
                            p.add_text(component_labels[i_component], font_size=30, position=(fig_width_per_component * 0.17,fig_height_per_row * (0.84 if n_rows == 3 else 0.88)))
                        ...
                        scalar_bar_args = dict(
                            ...
                            title= f'{sub_plot_no}',
                            position_x=0.2, 
                            position_y=0.15 if n_rows == 3 else 0.01,
                        )
                        ...
                        if i_component == 0 and i_row == 0:
                            p.add_axes(viewport=(-0.10, 0.54, 0.30, 0.94),xlabel='Z', ylabel='X', zlabel='Y', color='black')
...
def get_l2_and_l2_of_l2_norms(true_output,predictions):
    ...
    # l2_l2_norms_per_history_node_component_to_component = np.zeros(n_components)
    ...
    for i_component in range(n_components):
        # l2_l2_norms_per_history_node_component_to_component[i_component] = get_l2_norm_by_sqrt_n(l2_norms_per_history_node_component[:,:,i_component])
    ...
    # to be commented    
    # l2_l2_norms_per_history_node_component_to_history_component = np.zeros((n_histories, n_components))
    l2_l2_norms_per_history_time_component_to_history_component = np.zeros((n_histories, n_components))
    for i_history in range(n_histories):
        for i_component in range(n_components):
            # l2_l2_norms_per_history_node_component_to_history_component[i_history,i_component] = get_l2_norm_by_sqrt_n(l2_norms_per_history_node_component[i_history,:,i_component])
    ...
    # to be commented    
    # l2_l2_norms_per_history_node_component_to_history_component_to_component = np.zeros(n_components)
    l2_l2_norms_per_history_time_component_to_history_component_to_component = np.zeros(n_components)
    for i_component in range(n_components):
        # l2_l2_norms_per_history_node_component_to_history_component_to_component[i_component] = get_l2_norm_by_sqrt_n(l2_l2_norms_per_history_node_component_to_history_component[:,i_component])
    ...
    l2_norms = {}
    # l2_norms['l2_norms_per_history_node_component'] = l2_norms_per_history_node_component
    ...
    # l2_norms['l2_l2_norms_per_history_node_component_to_component'] = l2_l2_norms_per_history_node_component_to_component
    ...
    # l2_norms['l2_l2_norms_per_history_node_component_to_history_component'] = l2_l2_norms_per_history_node_component_to_history_component
    ...
    # l2_norms['l2_l2_norms_per_history_node_component_to_history_component_to_component'] = l2_l2_norms_per_history_node_component_to_history_component_to_component
...
def inspect_results_for_model_and_dataloader(training_data_dict, model_path, dataloader, description , mode):
    ...
    for history_index in history_indices:
        ...
        try:
            plot_true_vs_prediction_3D(unflattened_true_output[history_index], unflattened_predictions[history_index], history_index, description, trial_save_number, plots_save_folder)
        except Exception as e:
            my_logger.add_to_wait_list(f'Error while plotting 3D results: {e}')
            my_logger.log_and_print_the_wait_list()
            print(f'Error while plotting 3D results: {e}')
