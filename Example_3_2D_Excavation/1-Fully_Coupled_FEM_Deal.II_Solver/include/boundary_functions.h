...
namespace boundary_functions_space
{
    ...
    void BoundaryNeumannProvider<dim>::vector_value(const Point<dim> &p, Vector<double> &values) const
    {
        ...
        values = 0;
        if (std::fabs(p[1] - 28.0) < 1e-9)
        {
            double k_hydraulic_conductivity = 0.0001; 
            double ref_H_grad_magnitude = -0.42e00;   
            double H_gradient = ref_H_grad_magnitude * current_lc_data[t_step_no][0]; 
            values(dim) = -k_hydraulic_conductivity * H_gradient;
        }
    }
