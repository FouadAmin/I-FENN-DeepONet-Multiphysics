...
DoubleVector3D read_npy_file_as_DoubleVector3D(const std::string &file_path)
{
    cnpy::NpyArray arr = cnpy::npy_load(file_path);
    double *loaded_data = arr.data<double>();

    DoubleVector3D matrix(arr.shape[0], DoubleVector2D(arr.shape[1], DoubleVector1D(arr.shape[2])));
    for (size_t i = 0; i < arr.shape[0]; ++i)
    {
        for (size_t j = 0; j < arr.shape[1]; ++j)
        {
            ...
            {
                matrix[i][j][k] = loaded_data[i * arr.shape[1] * arr.shape[2] + j * arr.shape[2] + k];
            }
        }
    }
    return matrix;
