...
import shutil
import Module_DL_Helper_Excav_004 as MDLH
...
epochs_sessions_list = [100,1900,2000,2000,2000]
device_save_name = 'dev1'
general_model_name = 'mPE001-V002'
...
last_model_path=f'./{models_save_folder}/{general_model_name}_{trial_save_text}_last_train.pth'
best_model_path_train=f'./{models_save_folder}/{general_model_name}_{trial_save_text}_best_train.pth'
best_model_path_val = f'./{models_save_folder}/{general_model_name}_{trial_save_text}_best_val.pth'

previous_model_path = f'./{models_save_folder}/{general_model_name}_{trial_save_text}_last_train.pth' # if extend_on_previous_model is True, it will load the model from this path
# remove "EX_" from previous_model_path
previous_model_path = previous_model_path.replace('EX_','_')
...
if is_read_only:
    ...
    plots_save_folder = f'{parent_plots_save_folder}/{general_model_name}_{read_save_text}'
    sco_file_path = f'./{sco_save_folder}/{general_model_name}_{read_save_text}_sco.py'
    log_file_name = f'{general_model_name}_{read_save_text}_log.log'
else:
    # new model
    plots_save_folder = f'{parent_plots_save_folder}/{general_model_name}_{trial_save_text}'
    sco_file_path = f'./{sco_save_folder}/{general_model_name}_{trial_save_text}_sco.py'
    log_file_name = f'{general_model_name}_{trial_save_text}_log.log'
...
global_trunk_input_tensor = my_dataset.get_coordinates_array_list(return_as_tensor=True,normalize=True)
global_trunk_input_tensor_denormalized = my_dataset.denormalize_coord_tensor(global_trunk_input_tensor)
...
target_coord_list = []
target_coord_list.append(np.array([22, 26]))
target_coord_list.append(np.array([52, 21]))
target_coord_list.append(np.array([22, 6]))
target_coord_list.append(np.array([52, 6]))
target_coord_list.append(np.array([dim_x_a1, dim_y_a1])) # Bottom Left
target_coord_list.append(np.array([dim_x_c2, dim_y_a1])) # Bottom Right
target_coord_list.append(np.array([dim_x_a1, dim_y_a2])) # Top Left Zone a
target_coord_list.append(np.array([dim_x_a2, dim_y_a2])) # Top Right Zone a
target_coord_list.append(np.array([dim_x_a2, dim_y_b2])) # Top Left Zone b
target_coord_list.append(np.array([dim_x_c1, dim_y_b2])) # Top Right Zone b
target_coord_list.append(np.array([dim_x_c1, dim_y_c2])) # Top Left Zone c
target_coord_list.append(np.array([dim_x_c2, dim_y_c2])) # Top Right Zone c


computed_index_list = my_dataset.get_index_of_coordinates_list(target_coord_list)
computed_dim_list = global_trunk_input_tensor_denormalized[computed_index_list, :]
...
if plot_results_for_testing:
    my_logger.add_to_wait_list(f'----------------------------------------------------------------------------------')
    my_logger.add_to_wait_list(f'Testing Dataset')
    my_logger.add_to_wait_list(f'----------------------------------------------------------------------------------')
    ...
    tst_trunk_input_tensor = tst_dataset.get_coordinates_array_list(return_as_tensor=True,normalize=True)
    tst_trunk_input_tensor_denormalized = tst_dataset.denormalize_coord_tensor(tst_trunk_input_tensor)
    my_logger.add_to_wait_list(f'Testing: shape of the points_coordinates_tensor: {tst_trunk_input_tensor.shape}')

    # print original shapes
    my_logger.add_to_wait_list(f'Testing: original_load_grid_shape: {tst_dataset.original_load_grid_shape}')
    my_logger.add_to_wait_list(f'Testing: original_strain_grid_shape: {tst_dataset.original_strain_grid_shape}')
    my_logger.add_to_wait_list(f'Testing: original_output_grid_shape: {tst_dataset.original_output_grid_shape}')
    ...
    tst_computed_dim_list = tst_trunk_input_tensor_denormalized[tst_computed_index_list, :]
    for i in range(len(target_coord_list)):
...
# scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.97)
starting_lr = 1e-4
scheduler_config_list = [
    ('linear',100  ,200 , 1.0*starting_lr, 0.2*starting_lr),
    ('linear',300  ,500 , 0.2*starting_lr,0.04*starting_lr),
    ('linear',1000 ,1100,0.04*starting_lr,0.02*starting_lr),
    ('linear',4000 ,8000,0.02*starting_lr,0.01*starting_lr),
]
