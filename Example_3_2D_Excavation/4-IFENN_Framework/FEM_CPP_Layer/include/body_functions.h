...
namespace body_functions_space
{
    ...
    {
    public:
        BodyForcesProvider(const double t_current, const double t_dt, const int t_step_no);
    ...
    template <int dim>
    BodyForcesProvider<dim>::BodyForcesProvider(const double t_current, const double t_dt, const int t_step_no)
    ...
    inline void BodyForcesProvider<dim>::vector_value(const Point<dim> & /*p*/, Vector<double> &values) const
    {
        ...
        double mass_density = 2000.0; // kg/m^3
        double gravity = 10.0;        // m/s^2

        values = 0;
        values(1) = - mass_density * gravity;
    }
