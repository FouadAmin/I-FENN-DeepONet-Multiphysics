...
def plot_true_vs_prediction_2D(true_output, predictions, coordinates_list, grid_to_list_map, history_index, description, trial_save_number, plots_save_folder):
    ...
    component_labels = [' p  ', ' Ux ', ' Uy ']
    sub_plot_no_list = ['(a)', '(b)', '(c)', '(d)', '(e)', '(f)', '(g)', '(h)', '(i)', '(j)', '(k)', '(l)', '(m)', '(n)', '(o)', '(p)', '(q)', '(r)', '(s)', '(t)', '(u)', '(v)', '(w)', '(x)', '(y)', '(z)']

    time_indices_to_plot = [0,1,4,14,29,39,49,53,54,59]
            ...
            for plot_option in plot_options:
                ...
                for i_row in range(n_rows):
                    for i_col in range(n_cols):
                        ...
                        scalar_bar_pos_x = 0.71
                        scalar_bar_pos_y = 0.1
                ...
                cam_offset = 13.1  # adjust how far you want to move to the right
                focal_offset = cam_offset  # adjust how far you want to move the focal point
                ...
                save_name = f'b-2d_{description}_hist_{history_index}_{save_index_append_zeros}_time_{relative_t_index}_{add_name}.png'             
