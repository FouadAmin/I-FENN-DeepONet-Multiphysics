...
import shutil
import Module_DL_Helper_Excav_006 as MDLH
import Module_Solution_Loader_Helper_Excav_002 as MSLH
import mmap
import posix_ipc
import struct
import time
...
def load_shared_memories():
    ...
    shared_input_format = "2400f"  # n floats 
    shared_output_format = "2501f"  # n floats
    shared_check_format = "2400f"  # n floats
    ...
    if shm_fd0.size != mem_size_flags:
        raise ValueError("Shared memory size mismatch @ shm_fd0")
    if shm_fd1.size != mem_size_input:
        raise ValueError("Shared memory size mismatch @ shm_fd1")
    if shm_fd2.size != mem_size_output:
        raise ValueError("Shared memory size mismatch @ shm_fd2")
    if shm_fd3.size != mem_size_check:
        raise ValueError("Shared memory size mismatch @ shm_fd3")
...
def start_input_out_process():
    print("Starting the input/output process ... ")
    ...
    while True:
        ...
        input_tensor_grid = torch.tensor(input_vector).reshape(1, n_cells_x, n_cells_y, 1) # add time dimension to the beginning
        
        shared_check.seek(0)
        check_data = shared_check.read(struct.calcsize(shared_check_format))
        check_vector_raw = struct.unpack(shared_check_format, check_data)
        check_vector_grid = torch.tensor(check_vector_raw).reshape(1, n_cells_x, n_cells_y, 1) # add time dimension to the beginning
        ...
        #######################################################################
        input_tensor , _ , _ = MDLH.map_array_from_grid_to_list(input_tensor_grid, my_dataset.map_cells_grid_to_list_original, deal_ii_to_DeepONet_strain_skip[0], deal_ii_to_DeepONet_strain_skip[1])
        input_tensor = input_tensor.squeeze(-1) # remove the last dimension
        
        check_vector_list , _ , _ = MDLH.map_array_from_grid_to_list(check_vector_grid, my_dataset.map_cells_grid_to_list_original, deal_ii_to_DeepONet_strain_skip[0], deal_ii_to_DeepONet_strain_skip[1])
        check_vector_list = check_vector_list.squeeze() 
        check_tensor = torch.tensor(check_vector_list, dtype=torch.int32)
        max_check = torch.max(check_tensor)
        min_check = torch.min(check_tensor)
        check_vector_zeros_count = torch.sum(check_tensor == 0)
        check_vector_ones_count = torch.sum(check_tensor == 1)
        print(f"check_vector_zeros_count: {check_vector_zeros_count}, check_vector_ones_count: {check_vector_ones_count}")
        if not are_equal(max_check, 1.0) or not are_equal(min_check, 1.0):
            print(f"max_check: {max_check}, min_check: {min_check}")
            # loop over the check_tensor and print the non 1.0 values
            # open special log file to write the check_tensor values
            check_vector_log_file = f'check_vector_log.log'
            with open(check_vector_log_file, 'w') as f:
                for i, val in enumerate(check_tensor):
                    x_index = i // (n_cells_y)
                    y_index = (i % (n_cells_y)) 
                    f.write(f"check_tensor[{i}]=>[{x_index},{y_index}]: {val}\n")
            
            raise ValueError("Check vector is not 1.0")
        
        #######################################################################
        my_logger.add_to_wait_list(f"Received check_tensor => size: {check_tensor.shape} max: {max(check_tensor)} min: {min(check_tensor)}")
        ...
        my_logger.add_to_wait_list(f"Received input_tensor_grid shape: {input_tensor_grid.shape}")
        ...
        denormalized_output_tensor_back_to_grid =  MDLH.map_array_from_list_to_grid(denormalized_output_tensor, my_dataset.map_nodes_list_to_grid_current, my_dataset.original_coordinates_array_grid.shape[0], my_dataset.original_coordinates_array_grid.shape[1])
        my_logger.add_to_wait_list(f'shape of denormalized_output_tensor_back_to_grid: {denormalized_output_tensor_back_to_grid.shape}')
        denormalized_output_vector = denormalized_output_tensor_back_to_grid.squeeze().reshape(-1)
        # convert to tuple of floats
        output_vector = tuple(denormalized_output_vector.tolist())
        ...
        if t_step_no == 60:
            ...
            strain_runtime_save_name = init_save_name +'_strain_runtime.npy'
            np.save(f'{to_use_npy_save_folder}/{strain_runtime_save_name}', runtime_strain_data_up_to_current.cpu().numpy())
            strain_stored_save_name = init_save_name +'_strain_stored.npy'
            np.save(f'{to_use_npy_save_folder}/{strain_stored_save_name}', stored_strain_data_up_to_current.cpu().numpy())
            ...
            args_load_case_file_name_lc = args_load_case_file_name.replace('lcxxx',f'load{args_load_case_index}')
            all_steps_name = args_load_case_file_name_lc.replace('stxxx','all-steps')
            time_step_list = np.arange(1, 61, 1)
            n_time_steps = len(time_step_list)
            dealii_disp_array_grid = np.zeros((n_time_steps, 61, 41, 2))
            coords_grid_i = None
            for ii, time_index in enumerate(time_step_list):
                ...
                data_array_i, coords_grid_i = MSLH.custom_get_data(args_load_case_file_name_lc, time_index, False, False, fem_readpath)
            ...
            deepOnet_theta_array_list = predicted_output_data_up_to_current_denormalized.cpu().numpy()
            deepOnet_theta_array_grid = MDLH.map_array_from_list_to_grid(deepOnet_theta_array_list, my_dataset.map_nodes_list_to_grid_current, my_dataset.original_coordinates_array_grid.shape[0], my_dataset.original_coordinates_array_grid.shape[1])
            # concatenate the two arrays
            combined_predicted_array_grid = np.concatenate((deepOnet_theta_array_grid, dealii_disp_array_grid), axis=-1)
            # load true Data
            full_data_dict = p_dict['full_data_dict']
            output_data_i_full_denormalized = full_data_dict['output_data_i_full_denormalized']
            combined_true_array_list = output_data_i_full_denormalized.cpu().numpy()
            combined_true_array_grid = MDLH.map_array_from_list_to_grid(combined_true_array_list, my_dataset.map_nodes_list_to_grid_current, my_dataset.original_coordinates_array_grid.shape[0], my_dataset.original_coordinates_array_grid.shape[1])
            save_name_true = init_save_name +'_true.npy' 
            save_name_pred = init_save_name +'_pred.npy'
            np.save(f'{to_use_npy_save_folder}/{save_name_true}', combined_true_array_grid)
            np.save(f'{to_use_npy_save_folder}/{save_name_pred}', combined_predicted_array_grid)
            # plot true vs predicted slices
            
            combined_predicted_array_list, _, _ = MDLH.map_array_from_grid_to_list(combined_predicted_array_grid, my_dataset.map_nodes_grid_to_list_original, deal_ii_file_read_output_skip[0], deal_ii_file_read_output_skip[1])
            
            
            print(f'shape of combined_true_array_grid: {combined_true_array_grid.shape}')
            print(f'shape of combined_predicted_array_grid: {combined_predicted_array_grid.shape}')
            
            print(f'shape of combined_true_array_list: {combined_true_array_list.shape}')
            print(f'shape of combined_predicted_array_list: {combined_predicted_array_list.shape}')
            
            
            coordinates_list = my_dataset.get_coordinates_array_list(return_as_tensor=False, normalize=False)
            
            MDLH.plot_true_vs_prediction_2D(combined_true_array_list, combined_predicted_array_list, coordinates_list, my_dataset.map_nodes_grid_to_list_current, p_dict["target_load_case_index"], init_save_name, 'I-FENN', to_use_plots_save_folder)
            # ensure       
            cleanup()
            exit()
...
def load_data_for_a_specific_load_case(load_case_index):
    ...
    load_file_name = f'generated_excav_flux_data_v002_interp_60'
    output_file_name = f'all_nodes_array_grid_LC++-'
    strain_file_name = f'all_cells_array_grid_LC++-'

    map_nodes_grid_to_list_file_name = 'map_nodes_grid_to_list_LC++-0-100'
    map_cells_grid_to_list_file_name = 'map_cells_grid_to_list_LC++-0-100'

    coordinates_file_name = f'coordinates_nodes_grid_LC++-'
    normalization_option_coords = 2 # 0: no normalization, 1: normalize between 0 and 1, 2: normalize between -1 and 1 3: standardization 4: divide by max value 5: divide by std
    ...
    loss_criterion_option = 4 # 1 : MSELoss('mean') 2 : L1Loss('mean') 3 : SmoothL1Loss('mean') 4 : MSELoss('sum') 5 : L1Loss('sum') 6 : SmoothL1Loss('sum') 7 : CustomLoss (L2) 8 : CustomLoss (Normalized L2)
    trial_save_number = 'A018_A017+LargerDataset600'
    ...
    epochs_sessions_list = [100,900,1000]
    ...
    slicing_settings = {'load_time_start': 1,
                        ...
                        'strain_time_skip': 1,
                        'strain_time_end': -1,
                        'point_strain_start': (0,0,0), # (angle,radius,z)
                        'point_strain_skip': (2,2,2), 
                        'point_strain_end': (None,None,None),
                        'point_strain_flatten': True,
                        'point_output_start': (0,0,0), # (angle,radius,z)
    ...
    MDLH.train_for_pressure_only = train_for_pressure_only
    ...
    "map_nodes_grid_to_list_file_name": map_nodes_grid_to_list_file_name,
    "map_cells_grid_to_list_file_name": map_cells_grid_to_list_file_name,
    "LC_start": LC_start,
    "LC_end": LC_end,
    "slicing_settings": slicing_settings,
    "normalization_option": normalization_option,
    "normalization_option_coords": normalization_option_coords,
    ...
    ## Load Full Data
    MDLH.train_for_pressure_only = False # temporary change to load data
    ...
    hidden_temporal_size_a = 64
    hidden_temporal_size_b = 64
    temporal_num_layers_a = 2
    temporal_num_layers_b = 2


    trunk_input_dim = 2  # coordinate_x, coordinate_y, coordinate_z
    trunk_internal_dim = 256

    branch_output_dim_a = 64
    branch_output_dim_b = 64
    ...
    class DeepONet(nn.Module):
        def __init__(self, branch_input_dim,hidden_temporal_size, temporal_num_layers, branch_output_dim,trunk_input_dim, trunk_output_dim , branch_n_components, trunk_n_components):
            ...
            if selected_input_option == InputOptions.Load:
                ...
                self.branch_net_b = None
            elif selected_input_option == InputOptions.Strain:
                self.branch_net_a = None
            ...
            self.bc1_pressure_bc_value = MDLH.normalize_data(0 , my_dataset.var0_stats, normalization_option)
        ...
        def forward(self, load_input, strain_input ,trunk_input):
            ...
            if enforce_BC:
                # raise error("enforce_BC is not implemented yet")
                raise NotImplementedError("enforce_BC is not implemented yet")
            else:    
                return NN_output
...
def update_pytorch_dict_for_load_case(load_case_index):
    ...
    trunk_input_tensor = my_dataset.get_coordinates_array_list(return_as_tensor=True,normalize=True)
...
mesh_refinement = 0
n_cells_x = int(60 * (2**mesh_refinement))
n_cells_y = int(40 * (2**mesh_refinement))
n_nodes_x = int(n_cells_x + 1)
n_nodes_y = int(n_cells_y + 1)
deal_ii_to_DeepONet_strain_skip = (2, 2)
deal_ii_file_read_output_skip = (1, 1)
shm_object_dict = {} 
load_case_data = {}
p_dict = {}
noise_scale = 0.0
...
args_load_case_file_name = f'ExcavPE002_LoadV001_lcxxx_mesh{mesh_refinement}_t3000000_inc60_dt50000_stxxx.h5'
